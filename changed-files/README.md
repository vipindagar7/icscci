...
- **Speakers** and **Registration fees** — the source docx says "To be updated soon" for both; `speakers.list` is an empty array (renders a clean "to be announced" card). Add real speakers like:
  ```json
  { "name": "Dr. Jane Doe", "affiliation": "IIT Delhi", "topic": "Keynote title", "profile": "/images/speakers/jane-doe.jpg" }
  ```
  Leave `"profile": ""` (or omit it) for speakers without a photo yet — `PlaceholderImage` shows a labeled placeholder instead of crashing. It only ever renders a real `next/image` when `profile` starts with `/`, `http://`, or `https://`.
